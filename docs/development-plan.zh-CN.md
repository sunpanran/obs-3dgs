...
- 已实现：原生来源、私有 Browser Source、安全本地服务器、Spark 渲染、六类属性页、Dock、镜头预设、热键、双语界面。
...
- 2026-09-04 发布条件调整：用户负责 Windows 七项前端人工验收，通过后可公开 `0.1.0-beta.1`。macOS 实机验证可在 Beta 发布后补做，README 和发布说明必须明确写出尚未验证；这不代表 Mac 安装、渲染或 M1 性能已经通过。中英文 README 必须提供下载入口、分平台安装步骤及测试场景。
...
- Browser Source 缺失、服务器失败或 WebGL2 不可用时显示内置双语错误纹理。
...
- 卸载：关闭 OBS 后删除插件文件夹。
- 不制作 `.exe`、`.msi` 或 `.pkg`。
- 每个 ZIP 提供 SHA-256、中英文安装说明和未签名警告。
...
- 原生语言文件为 `en-US` 和 `zh-CN`。
- 默认读取 `obs_get_locale()`。
- 插件级语言选项：
  - 自动
  - 简体中文
  - English
- 覆盖选择同时应用于属性页、Dock、加载层、错误和工具提示。
- Dock 可即时重译；已打开的属性页提示重新打开。
- 英文是缺失翻译的回退语言。
- CI 检查两种语言 key 完全一致。
- `SH`、`LOD`、等效焦段和坐标系提供双语工具提示。
- 提供英文 README、中文 README 和两套安装/故障排查文档。[OBS 本地化机制](https://docs.obsproject.com/plugins#localization)
...
- macOS Universal ZIP。
- 不生成安装器和 Linux 包。
- 发布 SHA-256、SBOM、第三方许可证、双语说明和已知限制。
