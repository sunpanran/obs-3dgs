...
Spark is consumed as the published npm artifact with its included MIT license and complete notice. Upstream Rust Cargo metadata separately declares `Proprietary`, conflicting with the repository MIT declaration; clarification remains open in [upstream #402](https://github.com/sparkjsdev/spark/issues/402). This project does not copy/recompile those Rust sources or bundle the offline Rust converter. Any future source integration requires separate verification.
